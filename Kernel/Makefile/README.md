...
**注：参考链接（https://seisman.github.io/how-to-write-makefile/overview.html）**
...
prerequisites：target所需要的依赖文件和/或target

command：target要执行的命令（任意的shell命令）

有一个点就是如果说prerequisites：中的文件比target新了，就需要重新执行command定义的命令，**这是一个文件的依赖关系，也就是说，target这一个或多个的目标文件依赖于prerequisites中的文件，其生成规则定义在command中**，这就是makefile的规则，也就是makefile中最核心的内容
...
1. 1-2行，edit就是对应target，冒号后面的 `*.o ` 是中间目标文件其中每个中间目标文件也都有一组依赖文件，而这些 `\*.o` 文件又是执行文件 `edit` 的依赖文件。依赖关系的实质就是说明了目标文件是由哪些文件生成的，换言之，目标文件是哪些文件更新的。然后反斜线是换行的意思
2. 3-4行，对应着要执行的命令
3. 6-21行，`*.o`后面的冒号对应的prerequisites，也就是对应的依赖文件
4. 22-24行，clean冒号后面的是对应要执行的命令，需要说明的一点是clean不是一个文件，而clean相当于一个函数名一样的东西，make clean也就是相当于调用了这个函数（可以这样理解），然后还有一点需要注意的clean的冒号后面是什么都没有的，执行的命令是另起一行的，这个是因为如果冒号后面没有东西make不会去自动去找它的依赖性，也就不会自动执行其后定义的命令

还有一个需要注意的地方就是在定义好依赖关系后，后续的那一行定义了如何生成目标文件的操作系统命令，一定要以一个 `Tab` 键作为开头。记住，make并不管命令是怎么工作的，他只管执行所定义的命令。make会比较targets文件和prerequisites文件的修改日期，如果prerequisites文件的日期要比targets文件的日期要新，或者target不存在的话，那么，make就会执行后续定义的命令。
...
2. 如果找到了，就查找文件中的第一个target，用上面的例子举例也就是edit文件，并且把edit作为最终的目标文件
3. 如果edit不存在，或者说edit冒号后面的.o文件更新时间比edit新，就需要重新执行edit后面的命令来重新生成edit这个命令
4. 如果说.o文件不存在，就去寻找.o文件需要的依赖文件，然后调用.o文件后面的命令来重新生成.o文件
5. 当.o文件经过步骤4被补全了也就是不缺少.o文件了，就会利用.o文件来生成make的终极目标，也就是可执行文件edit
...
可以看到很繁琐，如果说想要加一个依赖文件，就需要至少在两行中添加内容，虽然说例子中的这个makefile文件没有那么多内容，可是如果面对一个大型的工程文件，那就需要很小心仔细的在应该添加内容的地方添加内容（这时候添加的地方肯定就不是两处了），所以出错的概率很大，所以为了更好多维护整个makefile，我们就可以定义一个变量，变量的内容就是这些依赖文件，变量的名字也都是可以自己决定的，还是以上面的例子举例，对应的定义一个名为objects的变量，简化后的makefile就应该是这样的：
...
其中1-2行就是对生成的变量进行一个定义，然后如果说我现再想添加一个依赖文件，就可以只修改一行的内容就可以了

### 让make自动推导

GNU的make很强大，它可以自动推导文件以及文件依赖关系后面的命令，于是我们就没必要去在每一个 `.o` 文件后都写上类似的命令，因为，我们的make会自动识别，并自己推导命令。

比如说上面的命令中main.o后面对应着main.c，kbd.o后面对用着kbd.c一样，然后自动推导可以自动推导这个内容，简单点说就是 `main.o : main.c defs.h` 与 `main.o : defs.h` 是等价的，而且main.o后面的命令也可以直接自动推导出来，所以利用自动推导机制再简化上面的makefile之后就是这样的：
...
这种方法，也就是make的“隐晦规则”。上面文件内容中， `.PHONY` 表示 `clean` 是个伪目标文件（之后会提到）。

### 另类风格的makefiles

既然我们的make可以自动推导命令，那么我看到那堆 `.o` 和 `.h` 的依赖就有点不爽，那么多的重复的 `.h` ，能不能把其收拢起来，好吧，没有问题，这个对于make来说很容易，下面看一下最新风格的makefile：
...
**这个文章来源的作者不建议使用这种方法，个人也觉着这样的方法不好，不友好，虽然说省略了篇幅但是对应着依赖关系就有些难以弄懂了**
...
2. 如果目录 `<prefix>/include` （一般是： `/usr/local/bin` 或 `/usr/include` ）存在的话，make也会去找（这句话翻译过来也就是 `/usr/local/bin` 或 `/usr/include`存在的话make也会去找）。
...
foo.o是目标文件，foo.c和defs.h是依赖文件，第二行cc是命令，并且命令是需要以tab键为开头的，这个规则告诉我们两件事：
...
targets是文件名（目标文件），以空格分开，可以使用通配符。一般来说，我们的目标基本上是一个文件，但也有可能是多个文件。

prerequisites是依赖文件，如果依赖文件中的某个文件比targets新，就需要重新执行命令来生成目标文件
...
如果我们想定义一系列比较类似的文件，我们很自然地就想起使用通配符。make支持三个通配符： `*` ， `?` 和 `~` 。这是和Unix的B-Shell是相同的。
 ...
 `~` ：这个通配符举个例子更容易理解， `~/test`，这个代表当前用户根目录下的test文件，假如当前用户是resery，那么  `~/test`等价于`home/resery/test`不过这个等价前面半部分的内容是根据你用户的$HOME所决定的，假如 `~` 之后不是 `/`而是一些字符的话，他就会把这个字符当成用户名，去寻找对应用户根目录下的test文件，举个例子就是 `~hchen/test`等价于   `/home/hchen/test`，和上一个例子一样前半部分的/home/hchen是由$HOME所决定的

通配符代替了你一系列的文件，如 `*.c` 表示所有后缀为c的文件。一个需要我们注意的是，如果我们的文件名中有通配符，如： `*` ，那么可以用转义字符 `\` ，如 `\*` 来表示真实的 `*` 字符，而不是任意长度的字符串。

其中*和%还是有区别的，用下面的例子举例来看
...
这个命令乍一看是没什么问题的，但是make是会报错的，报错的原因是因为*是shell支持的通配符，然后他在print后面使用了\*，这个通配符是识别不了的，因为这里只能识别的是makefile中的通配符，并且这个\*号不会按照预期展开而是直接识别为\*，它也就会直接去找名字是\*.c的这个文件，下面的lpr那行命令也是一样的道理（下面那行前面有一个tab键也就证明这是shell命令，而$?（这是一个自动化变量）是makefile中的通配符，shell也识别不了），所以使用通配符的时候需要确定是makefile的通配符还是shell的通配符

```
objects = *.o
```

这个例子就是对应上面解释的\*.o不会展开，而会直接识别成符号*

不过你也可以使用*作为通配符，不过需要使用另一种方法，其中Makefile的变量其实就是C/C++中的宏。如果你要让通配符在变量中展开，也就是让objects的值是所有 `.o` 的文件名的集合，那么，你可以这样：
...
上面的两个代码框里，第二行中%后面没有接别的东西是空的，具体代表什么还没由弄清楚，需要再补充
...
$(filter %.o,$(files))表示调用Makefile的filter函数，过滤“$files”集，只要其中模式为“%.o”的内容。这个进行展开之后就应该是这样的
...
这个规则的意思是，所有的 `.d` 文件依赖于 `.c` 文件，`@set -e`中的@他的意思是执行这条命令但不显示，然后`set -e`是应该在咱们自己编写的所有的脚本文件中都应该写在头部，他会检测，如果有哪条语句返回非真实的返回值，就退出这个脚本， `rm -f $@` 的意思是删除所有的目标，也就是 `.d` 文件，第二行的意思是，为每个依赖文件 `$<` ，也就是 `.c` 文件生成依赖文件， `$@` 表示模式 `%.d` 文件，如果有一个C文件是name.c，那么 `%` 就是 `name` ， `$$$$` 意为一个随机编号，第二行生成的文件有可能是“name.d.12345”，第三行使用sed命令做了一个替换，关于sed命令的用法请参看相关的使用文档。第四行就是删除临时文件。
...
通常情况下，make会把执行的命令和执行命令的结果一块输出到屏幕上，不过有时我们只想让执行命令的结果输出到屏幕上而不输出执行的命令，这时候我们就可以在执行的命令前面加上一个 `@` 字符，加上之后make之后就不会输出执行的这个命令了只会输出执行命令之后的结果，可以看一下下面的这个例子：
...
可以很明显的看到差别，同时make加上`-n` 或 `--just-print` 参数，那么make执行后只会显示命令，但是不会执行命令

其实这个功能很方便我们来调试makefile，也就可以让我们看看我们书写的命令执行起来是什么样子的或者是什么顺序的

然后make加上 `-s` 或 `--silent` 或 `--quiet` 参数，就会全面禁止命令的显示

### 命令执行

这个命令执行可以与嵌套执行make（后面会涉及）联合使用

比如说我们一个工程文件里，不止含有一个makefile，其中主目录下的是主makefile，但是有些子目录里面也有makefile，而这是如果说你想在主makefile中执行make指令去执行子目录里面的makefile，按照正常的shell命令，就应该是先cd，然后make，但是如果说你是按照下面这样的顺序写的得话就不会得到预期的效果：
...
这样得到的效果就是make了主目录下的主makefile，所以这样的写法达不到预期的效果。如果遇到这种情况，想要用上一条命令的结果应用在下一条命令时，你就需要在上一条命令后面加上一个分号来分隔这两条命令。按照这个规则改一下上面的代码就是这样的：
...
这时候执行make exec就会进入到test目录里然后执行make命令
...
在执行命令的过程过中，命令执行结束之后，make会检测命令的返回码，如果命令返回成功，那么make才会执行下一条命令，当规则中所有的命令都成功返回后，这个规则就算是完成了。如果一个规则中的某个命令出错了（命令退出码非0），那么make就会终止当前规则，这将有可能终止所有规则的执行

但是有的时候命令虽然出错但是它出错是在预料之中的，在这种情况的时候我们就希望这条命令即使出错了也能继续执行下去，举个例子就是mkdir命令创建一个目录，如果说目录不存在mkdir创建成功，返回码也是正常的成功返回，但是如果说这个目录已经存在了，mkdir就会报错从而导致终止，但是我们的最终目标就是要用mkdir创建这个目录，也就是不存在创建，存在不做操作，而现再是不存在创建，存在报错。

所以为了解决这个问题我们需要在命令前加上一个减号 `-` （在Tab键之后），标记为不管命令出不出错都认为是成功的，如：
...
这里就和前面的命令执行呼应上了，在一些大的工程中，我们会把我们不同模块或是不同功能的源文件放在不同的目录中，我们可以在每个目录中都书写一个该目录的Makefile，这有利于让我们的Makefile变得更加地简洁，而不至于把所有的东西全部写在一个Makefile中，这样会很难维护我们的Makefile，这个技术对于我们模块编译和分段编译有着非常大的好处。
...
> += 是添加等号后面的值
>
> 不过根据上面的解释还是很难理解 = 和 += 的区别，不过用例子就可以很容易理解了：
>
> " "= "
>
> ```
...
> ```
>
> make会将整个makefile展开后，再决定变量的值。也就是说，变量的值将会是整个makefile中最后被指定的值。所以对应的x的值是xyz，y的值是xyz bar
>
> " := "
>
> ```
...
> ```
>
> " := "表示变量的值决定于它在makefile中的位置，而不是整个makefile展开后的最终值。所以x的值是xyz，y的值是foo bar
...
变量的命名可以包含字符、数字，下划线（可以是数字开头），但不应该含有 `:` 、 `#` 、 `=` 或是空字符（空格、回车等）。变量是大小写敏感的，“foo”、“Foo”和“FOO”是三个不同的变量名。传统的Makefile的变量名是全大写的命名方式，但我推荐使用大小写搭配的变量名，如：MakeFlags。这样可以避免和系统的变量冲突，而发生意外的事情。

有一些变量是很奇怪字串，如 `$<` 、 `$@` 等，这些是自动化变量，后面会介绍。

### 变量的基础

关于变量可以简化为两个步骤，一个声明，一个使用，其中声明的时候需要给予初值，使用的时候需要在变量名前加上 `$` 符号，不过最好用小括号 `()` 或是大括号 `{}` 把变量给包括起来。如果你要使用真实的 `$` 字符，那么你需要用 `$$` 来表示。
...
可以看到第2行就是变量在依赖中，第3行就是变量在命令中，第5行就是变量在目标中
...
和C语言一样我们也可以用变量给变量赋值，在在Makefile中有两种方式来在用变量定义变量的值。
...
这个就和之前在嵌套执行make中说的一样，= 运算符的规则是：make会将整个makefile展开后，再决定变量的值。也就是说，变量的值将会是整个makefile中最后被指定的值。

当我们再对上面的代码进行一下修改就会改变其结果，例如：
...
这时候执行make all就会输出 `aaa` 而不是之前的 `Huh?` 了
...
这会让make陷入无限的变量展开过程中去，当然，我们的make是有能力检测这样的定义，并会报错。还有就是如果在变量中使用函数，那么，这种方式会让我们的make运行时非常慢，更糟糕的是，他会使用的两个make的函数“wildcard”和“shell”发生不可预知的错误。因为你不会知道这两个函数会被调用多少次。
...
这部分上面的嵌套执行make中举的例子那里已经很清楚了，再加上这些例子就可以很容易的理解 `=` 和 `:=` 的区别了

上面都是一些比较简单的变量使用了，让我们来看一个复杂的例子，其中包括了make的函数、条件表达式和一个系统变量“MAKELEVEL”的使用：

**注：这个命令用现再的知识还无法解答，需要配合函数，条件表达式，等知识才可以理解，所以先留下，等后续知识学完再继续分析**
...
这个例子中， `$($($(z)))` 扩展为 `$($(y))` ，而其再次被扩展为 `$($(subst 1,2,$(x)))` 。 `$(x)` 的值是“variable1”，所以再进行扩展就是 `$($(subst 1,2,variable1)` ，subst函数把“variable1”中的所有“1”字串替换成“2”字串，于是，“variable1”变成 “variable2”，再取其值，所以，最终， `$(a)` 的值就是 `$(variable2)` 的值——“Hello”。（喔，好不容易）
...
define指示符后面跟的是变量的名字，而重起一行定义变量的值，定义是以endef 关键字结束。其工作方式和“=”操作符一样。变量的值可以包含函数、命令、文字，或是其它变量。因为命令需要以[Tab]键开头，所以如果你用define定义的命令变量中没有以 `Tab` 键开头，那么make 就不会把其认为是命令。**（这里有一个误解，我最开始看的时候以为它说的没有以Tab键开头指的是第2行和第3行没有以Tab键开头，然后我去试了一下这个命令，发现不管2、3行加不加Tab键开头都可以执行对应的命令，后来又仔细的阅读了一下这句话，其实这句话的意思是define定义的命令变量需要以Tab键开头才能被识别为是命令，意思就是two-lines在使用的时候需要加一个Tab才会被识别为命令）**
...
这个定义完的作用就是所有以.o为后缀的文件对应的CFLAGS就是-o
...
如果在make的命令行里	，你不只一次地使用了 `-f` 参数，那么，所有指定的makefile将会被连在一起传递给make执行。
...
我们还可以通过“模式规则”的方式写下自己的隐含规则。用“后缀规则”来定义隐含规则会有许多的限制。使用“模式规则”会更智能和清楚，但“后缀规则”可以用来保证我们Makefile的兼容性。我们了解了“隐含规则”，可以让其为我们更好的服务，也会让我们知道一些“约定俗成”了的东西，而不至于使得我们在运行Makefile时出现一些我们觉得莫名其妙的东西。当然，任何事物都是矛盾的，水能载舟，亦可覆舟，所以，有时候“隐含规则”也会给我们造成不小的麻烦。只有了解了它，我们才能更好地使用它。
...
这里我们将讲述所有预先设置（也就是make内建）的隐含规则，如果我们不明确地写下规则，那么，make就会在这些规则中寻找所需要的规则和命令。当然，我们也可以使用make的参数 `-r` 或 `--no-builtin-rules` 选项来取消所有的预设置的隐含规则。
...
1. 1-6行，定义了几个变量，一个是PROJ他的值是7（属于字符7），EMPTY和SPACE就是定义一个空变量和一个空格变量，SLASH就是一个斜线，V是一个@符号
2. 10-23行，定义了一个if判断，首先判断是否定义了GCCPREFIX，如果说没有定义才会进行下面的分支，首先会给GCCPREFIX变量赋值，赋值的内容是几个shell语句，关于shell语句就不分析了，不过这些shell大致的功能就是检测是否有编译32程序的能力
3. 26-38行，另一个if判断，也是会执行一些shell语句，这个会检测是否定义了QEMU，如果没定义才会执行下面的分支，shell的功能是检测qemu有没有运行32模式的能力
4. 41行，改变后缀列表的内容为.c、.S、.h
5. 44行，这个DELETE_ON_ERROR这篇文档里没有提到，查了一下这个是如果说编译失败，会把已经编译的内容删除掉
6. 47-59行，又一个if分支，如果没有定义了USELLVM，会设置一些变量，HOSTCC为gcc，HOSTCFLAGS为-g -Wall -O2，CC这里涉及到了一个函数$(GCCPREFIX)，这个其实就是找到gcc的前缀也就是路径比如说gcc的路径是/home/test/gcc，那么$(GCCPREFIX)就是/home/test/，然后是CFLAGS，CFLAGS赋值的两次，一次是赋值，一次是追加赋值，CFLAGS中涉及到了一个$(DEFS)不过这个没查到具体的作用是什么，然后如果不是的话就是使用clang，然后再更改一下CFLAGS
7. 61-86行，定义了一些变量，主要是定义了一些shell命令
8. 88行，包含进来了一个函数库文件，这个文件包含后面所有定义的函数是如何定义的
9. 90行，定义了一个listf_cc函数，其中又调用了另一个函数listf，其中listf函数的定义是在88行include进来的哪个函数库文件里，对应的定义是`listf = $(filter $(if $(2),$(addprefix %.,$(2)),%),$(wildcard $(addsuffix $(SLASH)*,$(1))))`从里往外看，里面有一个if判断，如果说参数2的值不是空，那就给参数2前面加上前缀%.，如果说参数2为空就不变，然后后面有一个扩展通配符也就是给参数1加上后缀/*然后，最后有一个过滤，其最终能达到的效果也就是把以参数1为目录名下的以参数2为后缀的文件全部过滤掉
10. 92行，定义一个变量
11. 95行，定义了一个add_files_cc函数，他调用了add_files函数，具体函数的定义都在function.mk里面，具体函数的功能就不分析了，因为具体分析也会浪费很多时间，咱们只是分析一下makefile的语法，具体他实现什么功能就不分析了
12. 96-111行，定义了一系列函数
13. 116-120行，追加赋值了三个变量
14. 122行，调用函数add_files_cc
15. 127-135行，追加赋值了三个变量，然后定义了一个空变量
16. 137-138行，调用了两个函数
17. 140行，给一个变量赋值
18. 142-151行，定义了一个uprog_ld命令集
19. 153行，利用了一个循环，调用了read_packet这个函数和uprog_ld定义的命令集
20. 158-181行，追加赋值
21. 183-188行，调用了几个函数，并且涉及到了一个重要的变量kernel，他的值为bin/kernel
22. 192-198行，就是第一个默认目标文件了，他依赖于$(KOBJS) $(USER_BINS)，然后回执行一些命令，不过除了第2行的命令都不会显示命令而是只显示输出，然后调用了create_target函数
23. 202-215行，是生成了另一个目标文件bootblock，具体就不分析了
24. 220-221行，是调用了两个函数add_files_host和add_files_host
25. 226-233行，是创建ucore.img文件
26. 238-243行，是创建swap.img文件
27. 270-289行，定义了几个伪目标，是为了方便调试的命令
28. 294行之后，后面的内容就是为了方便调试弄的伪目标

# 总结

刚开始没有了解makefile的时候看ucore的makefile一头雾水，但是看完之后理解的就容易一些了，并且ucore中也并没有涉及到文档中更深度一些的东西，所以理解起来还是可以的，不过关于shell命令里面的参数和格式之类的了解的还不够多需要加强
