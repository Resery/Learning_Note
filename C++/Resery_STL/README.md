...
二叉搜索树：这个很容易理解即一个结点的左子树中的每个结点一定小于这个结点，右子树中的每个结点一定大于这个结点。

二叉搜索树的增删：删除，就先看这个结点有几个子结点，如果只有一个子结点，直接把删除的结点移除，然后子结点直接连接删除结点的父节点即可，如果是两个子结点，则取该结点的右子节点中最小的值来替换他的位置即可。

平衡二叉搜索树：直接理解以下平衡的概念就可以了，平衡就是没有任何一个结点的深度过深，比如说左子树就只有一个结点深度即为2，右子树有比左子树多10倍的结点，这就会导致右子树的深度比左子树大很多，这就相当于不平衡。

AVL tree：任何左右子树高度相差最多1。

AVL tree增删：增删的时候可能会违反上面的定义，导致该树的高度相差超过了1，这时候就需要对他进行转换，分为单旋转和双旋转，理解单旋转和双旋转，看一下我画的图就差不多可以理解了。

![](https://resery-tuchuang.oss-cn-beijing.aliyuncs.com/2020-07-17_19-23-00.jpg)

RB-tree（红黑树）：红黑树也是平衡二叉搜索树的一种，而且他不仅是一个二叉搜索树，还需要满足几个规定

1. 根节点必须为黑色
2. 每个结点不是黑就是红
3. 红色的节点的子结点必须为黑色
4. 任意一个结点到达树的尾端的任何路径，所含的黑色结点数量是要相同的

RB-tree（红黑树）增删：红黑树的增删和AVL tree相比，也是进行旋转，然后比旋转多了一个变换颜色的步骤，看图的例子即可，例子只举了特殊的例子

![](https://resery-tuchuang.oss-cn-beijing.aliyuncs.com/2020-07-17_19-24-00.jpg)

### stl_pair

这个东西我还不太理解为什么设计这个东西，等写完map之类的就应该可以知道了，目前tree也用到了这个东西。pair在关联容器中经常被使用，它提供了两个成员变量first和second，由于pair是一个struct，所以其成员变量的属性是public。在pair struct中提供了构造函数和拷贝构造函数，同时提供了两个最基本的操作operator==和operator<重载，其他的操作符重载都是基于前面两种的变形。

### RB-tree的结点

RB-tree的结点即包含主要的五个元素，父结点，左子结点，右子结点，结点的具体数值，和黑（或红）。以及求最大和最小。

### RB-tree的迭代器

迭代器这里就采用了header技巧，header指向根节点的指针，与根节点互为对方的父节点。具体就如下图所示：

![](https://resery-tuchuang.oss-cn-beijing.aliyuncs.com/2020-07-17_12-39-00.jpg)

其中`_M_increment`和`_M_decrement`有对几个特殊情况的处理，即当前节点为根节点且根节点没有右子结点，还有当前节点为header。其余的部分代码里注释自我感觉写的挺详细的了，关于更改颜色以及旋转的那一块注释文字描述的很干涩，最好边阅读边画图才能更好的理解
